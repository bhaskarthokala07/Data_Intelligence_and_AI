...
def build_static_prompt(query: str, all_docs: List[Dict]) -> str:
...
The following prompt would be sent to Groq:
...
def main():
    ...
    if run and user_query.strip():
        ...
        tab1, tab2 = st.tabs(
            [
                "STATIC (All Documents)",
                "DYNAMIC (Smart Retrieval)"
            ]
        )
        ...
        with tab1:
            ...
            st.caption(
                f"Approx Tokens: {len(static_prompt.split())}"
            )
        ...
        with tab2:
            ...
            for item in retrieved:
                ...
                st.success(
                    f"✅ {item['title']} | Score: {item['relevance_score']}"
                )

            st.caption(
                f"Approx Tokens: {len(dynamic_prompt.split())}"
            )
        ...
        st.info(
            "Dynamic Context Injection sends only relevant information to the LLM, reducing token usage and improving response quality."
        )
